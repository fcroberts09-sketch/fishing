            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:14}}>
              ...
              <div style={{background:C.card,borderRadius:14,border:editMode?'2px solid '+C.amber:'1px solid '+C.bdr,overflow:'hidden',position:'relative'}}>
                ...
                <div style={{height:500,position:'relative'}}>
                  ...
                  {editPopup && editMode && <div style={{position:'absolute',right:10,top:52,zIndex:1000,width:280,background:C.card,border:'1px solid '+C.bdr2,borderRadius:14,boxShadow:'0 8px 32px #000a',overflow:'hidden'}} onClick={e=>e.stopPropagation()}>
                    <div style={{padding:'10px 14px',borderBottom:'1px solid '+C.bdr,display:'flex',justifyContent:'space-between',alignItems:'center',background:editPopup.type==='spot'?sc(editPopup.data.type)+'12':C.cyan+'12'}}>
                      <div style={{fontWeight:700,fontSize:13,display:'flex',alignItems:'center',gap:6}}><EditI s={14} c={editPopup.type==='spot'?sc(editPopup.data.type):C.cyan}/> Edit {editPopup.type==='spot'?'Spot':'Launch'}</div>
                    ...
                    <div style={{padding:14}}>
                      ...
                      <div style={{padding:'8px 0',background:C.card2,borderRadius:8,textAlign:'center',marginBottom:10,fontFamily:FM,fontSize:11,color:C.mid}}>
                        {editPopup.data.gps ? (typeof editPopup.data.gps === 'string' ? editPopup.data.gps : editPopup.data.gps.lat+', '+editPopup.data.gps.lng) : ''}
                      </div>
                      <div style={{display:'flex',gap:6}}>
                        <Btn primary small style={{flex:1}} onClick={()=>{showT('Saved!');setEditPopup(null);}}><SaveI s={12} c={C.bg}/> Done</Btn>
                        <Btn small danger onClick={()=>handleDeleteMarker(editPopup.type,editPopup.id)}><TrashI s={12}/> Delete</Btn>
                      </div>
                    </div>
                  </div>}
                </div>
