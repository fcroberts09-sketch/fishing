...
import { getRouteChoices } from './data/channelGraph';
...
export default function App() {
  ...
  const [routeChoice, setRouteChoice] = useState(null); // selected route id, e.g. 'west_bay_icw'
  const [routeChoiceOptions, setRouteChoiceOptions] = useState(null); // { zoneName, routes[] } or null
  ...
  const curRoute = useMemo(() => {
    ...
    const route = generateRoute(startLat, startLng, startName, sLat, sLng, selSpot.name, routeChoice);
    return computeRouteStats(route);
  }, [selSpot, selBay, bayConfig, routeKey, savedRoutes, launches, computeRouteStats, routeChoice]);
  ...
  const endNav = () => { setShowRoute(false); setRouteStep(0); setPlaying(false); setTripActive(false); setMobilePanel(null); setEditingRoute(false); setRouteChoice(null); };
  const startNav = () => {
    if (selSpot && selBay) {
      const [sLat, sLng] = itemToLatLng(selSpot, bayConfig);
      const choices = getRouteChoices(sLat, sLng);
      if (choices && choices.routes.length > 1 && !routeChoice) {
        setRouteChoiceOptions(choices);
        return;
      }
    }
    setShowRoute(true); setRouteStep(0); setPlaying(false); setTripActive(true); setTripStart(Date.now()); setMobilePanel('nav');
  };
  const confirmRouteChoice = (routeId) => {
    setRouteChoice(routeId);
    setRouteChoiceOptions(null);
    setShowRoute(true); setRouteStep(0); setPlaying(false); setTripActive(true); setTripStart(Date.now()); setMobilePanel('nav');
  };
...
Respond in this exact JSON format (no markdown, no code fences, just raw JSON):
{
  ...
  return (
    <div style={{ fontFamily: Fnt, background: C.bg, color: C.txt, minHeight: '-webkit-fill-available' }}>
      ...
      {routeChoiceOptions && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setRouteChoiceOptions(null)}>
        <div style={{ background: C.card, borderRadius: 14, padding: isMobile ? 20 : 24, maxWidth: 380, width: '100%', boxShadow: '0 8px 40px #000a', border: `1px solid ${C.bdr}` }} onClick={(e) => e.stopPropagation()}>
          <div style={{ fontWeight: 700, fontSize: 16, color: C.txt, marginBottom: 4 }}>Choose Route to {routeChoiceOptions.zoneName}</div>
          <div style={{ fontSize: 12, color: C.mid, marginBottom: 16 }}>{routeChoiceOptions.routes.length} routes available from the harbor</div>
          {routeChoiceOptions.routes.map((route, idx) => (
            <button key={route.id} onClick={() => confirmRouteChoice(route.id)} style={{ width: '100%', padding: '14px 16px', borderRadius: 10, border: idx === 0 ? `1px solid ${C.bdr}` : `1px solid ${C.cyan}60`, background: idx === 0 ? C.card2 : `${C.cyan}10`, cursor: 'pointer', marginBottom: 10, textAlign: 'left', fontFamily: Fnt }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: C.txt }}>{route.name}</div>
              <div style={{ fontSize: 11, color: C.mid, marginTop: 2 }}>{route.desc}</div>
              {route.warning && <div style={{ fontSize: 10, color: C.amber, fontWeight: 600, marginTop: 4 }}>{route.warning}</div>}
            </button>
          ))}
          <button onClick={() => setRouteChoiceOptions(null)} style={{ width: '100%', marginTop: 0, padding: '8px 0', background: 'none', border: 'none', color: C.dim, cursor: 'pointer', fontSize: 12, fontFamily: Fnt }}>Cancel</button>
        </div>
      </div>}
