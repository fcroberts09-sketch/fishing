...
function MapClickHandler({ onRightClick, onLeftClick, editMode }) {
  useMapEvents({
...
export default function App() {
  ...
  const [ctxMenu,setCtxMenu]=useState(null); // {lat, lng, x, y} for right-click menu
  const [editPopup,setEditPopup]=useState(null); // {type:'spot'|'launch'|'photo', id, data}
  ...
  const [drawingLine,setDrawingLine]=useState(null); // {points:[], label:''}
  const [editingZone,setEditingZone]=useState(null);
  const [editingLine,setEditingLine]=useState(null);
  const CAST_METERS = 40 * 0.9144; // 40 yards in meters
  ...
  // Compute cast line offsets (perpendicular to wade line, 40 yards each side)
  const getCastLineOffsets = (pts, rangeMeters) => {
    ...
    for (let i = 0; i < coords.length; i++) {
      ...
      const offsetLat = rangeMeters / mPerLat;
      const offsetLng = rangeMeters / (mPerLat * Math.cos(coords[i][0] * Math.PI / 180));
      left.push([coords[i][0] + nx * offsetLat, coords[i][1] + ny * offsetLng]);
      right.push([coords[i][0] - nx * offsetLat, coords[i][1] - ny * offsetLng]);
    }
    return { left, right };
  };
  ...
  const handleMapRightClick = (e) => {
    ...
    if (drawingLine) {
      ...
      showT('Point added (right-click to add more, click Finish to save)');
      return;
    }
  ...
  const handleAddFromCtx = (type) => {
    ...
    if (type === 'wade-line') {
      ...
      setCtxMenu(null);
      showT('Wade line started! Right-click to add points, then click Finish.');
      return;
    }
    if (type === 'wade-zone') {
      ...
      showT('Wade zone added - edit size and label');
      setCtxMenu(null);
      return;
    }
    if (type === 'launch-boat' || type === 'launch-kayak' || type === 'launch-drivein') {
      ...
      showT('Launch added - click to edit name');
    } else {
      const newS = { id, bay: selBay?.id || 'matagorda', name: 'New Spot', type, position: pos, gps, rating: 0, species: [], bestTide: 'Any', bestTime: '', bestSeason: '', bestWind: '', lures: [], desc: '', parking: pos, media: [], route: [] };
      setAllSpots(prev => [...prev, newS]);
      setEditPopup({ type: 'spot', id, data: newS });
      showT('Spot added - click to edit details');
    }
    setCtxMenu(null);
  };
  ...
  const handleFinishWadeLine = () => {
    ...
    const id = Date.now();
    setWadeLines(prev => [...prev, { id, bay: selBay?.id || 'matagorda', label: drawingLine.label, points: drawingLine.points, color: C.amber, castRange: 40 }]);
    setDrawingLine(null);
  ...
  const handleMarkerDragEnd = (markerType, id, e) => {
    ...
    const newPos = latLngToPosition(newLatLng.lat, newLatLng.lng);
    const newGps = posToGPSStr(newPos);
    if (markerType === 'spot') {
      setAllSpots(prev => prev.map(s => s.id === id ? { ...s, position: newPos, gps: newGps, parking: newPos } : s));
      if (selSpot?.id === id) setSelSpot(prev => prev ? { ...prev, position: newPos, gps: newGps } : prev);
      if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, position: newPos, gps: newGps } } : prev);
    } else if (markerType === 'launch') {
      setLaunches(prev => prev.map(l => l.id === id ? { ...l, position: newPos, gps: newGps.lat + ', ' + newGps.lng } : l));
      if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, position: newPos, gps: newGps.lat + ', ' + newGps.lng } } : prev);
      ...
      if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, cx: newPos.x, cy: newPos.y } } : prev);
    } else if (markerType === 'wade-pt') {
      // id is {lineId, ptIndex}
      setWadeLines(prev => prev.map(wl => wl.id === id.lineId ? { ...wl, points: wl.points.map((p, i) => i === id.ptIndex ? newPos : p) } : wl));
    }
    const [lat, lng] = bayConfig.toLatLng(newPos);
    showT('Moved to ' + lat.toFixed(5) + ', ' + lng.toFixed(5));
  };

  const handleUpdateSpot = (id, updates) => {
    setAllSpots(prev => prev.map(s => s.id === id ? { ...s, ...updates } : s));
    if (selSpot?.id === id) setSelSpot(prev => prev ? { ...prev, ...updates } : prev);
    if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, ...updates } } : prev);
  };

  const handleUpdateLaunch = (id, updates) => {
    setLaunches(prev => prev.map(l => l.id === id ? { ...l, ...updates } : l));
    if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, ...updates } } : prev);
  };

  const handleUpdateZone = (id, updates) => {
    setShadeZones(prev => prev.map(z => z.id === id ? { ...z, ...updates } : z));
    if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, ...updates } } : prev);
  };

  const handleUpdateWadeLine = (id, updates) => {
    setWadeLines(prev => prev.map(wl => wl.id === id ? { ...wl, ...updates } : wl));
    if (editPopup?.id === id) setEditPopup(prev => prev ? { ...prev, data: { ...prev.data, ...updates } } : prev);
  };
  ...
  const handleDeleteMarker = (markerType, id) => {
    ...
    } else if (markerType === 'zone') {
      setShadeZones(prev => prev.filter(z => z.id !== id));
    } else if (markerType === 'wadeline') {
      setWadeLines(prev => prev.filter(w => w.id !== id));
    } else if (markerType === 'photo') {
  ...
  // Zone center icon for dragging
  const zoneCenterIcon = (color) => L.divIcon({
    className: '',
    html: '<div style="width:20px;height:20px;border-radius:50%;background:' + color + ';border:2px solid #fff;opacity:0.8;cursor:move;box-shadow:0 2px 8px #0006;display:flex;align-items:center;justify-content:center;font-size:10px;color:#fff;font-weight:700">+</div>',
    iconSize: [20, 20], iconAnchor: [10, 10],
  });

  // Wade line point icon for dragging
  const wadePointIcon = () => L.divIcon({
    className: '',
    html: '<div style="width:14px;height:14px;border-radius:50%;background:' + C.amber + ';border:2px solid #fff;cursor:move;box-shadow:0 2px 6px #0006"></div>',
        ...
        {page==='bay'&&selBay&&!showBS&&(
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:14}}>
              {/* SATELLITE MAP */}
              <div style={{background:C.card,borderRadius:14,border:editMode?'2px solid '+C.amber:'1px solid '+C.bdr,overflow:'hidden',position:'relative'}}>
                <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.bdr}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div><div style={{fontSize:13,fontWeight:600}}>{showRoute?'Route \u2192 '+selSpot?.name:editMode?'Edit Mode \u2014 Right-click to add, drag to move':'Satellite Fishing Map'}</div><div style={{fontSize:11,color:editMode?C.amber:C.dim}}>{editMode?'Right-click: add marker \u2022 Click marker: edit \u2022 Drag: move':'Sentinel-2 / USGS / ESRI \u2014 Toggle layers top-right'}</div></div>
                  <div style={{display:'flex',gap:6}}>
                    <button onClick={()=>{setEditMode(!editMode);setCtxMenu(null);setEditPopup(null);}} style={{padding:'5px 12px',borderRadius:6,fontSize:11,fontWeight:700,background:editMode?C.amber:C.card2,color:editMode?C.bg:C.mid,border:`1px solid ${editMode?C.amber:C.bdr}`,cursor:'pointer',fontFamily:Fnt,display:'flex',alignItems:'center',gap:4}}><EditI s={13}/> {editMode?'Done Editing':'Edit Map'}</button>
                ...
                <div style={{height:500,position:'relative'}}>
                  <MapContainer center={bayConfig.center} zoom={bayConfig.zoom} style={{height:'100%',width:'100%'}} zoomControl={false} key={selBay.id}>
                    ...
                    <MapClickHandler onRightClick={handleMapRightClick} onLeftClick={() => { setCtxMenu(null); if(!editPopup) setEditPopup(null); }} editMode={editMode} />

                    {/* Shade zones */}
                    {!showRoute && bayShades.map(z=>(
                      <Polygon key={z.id} positions={shadeToPolygon(z)} pathOptions={{color:z.color,weight:editMode?2.5:1.5,dashArray:editMode?'':'6 4',fillColor:z.color,fillOpacity:editMode?0.2:0.12}} eventHandlers={{click:()=>{ if(editMode){setEditPopup({type:'zone',id:z.id,data:z});setCtxMenu(null);} }}}>
                    ...
                    {editMode && !showRoute && bayShades.map(z=>(
                      <Marker key={'zc'+z.id} position={bayConfig.toLatLng({x:z.cx,y:z.cy})} icon={zoneCenterIcon(z.color)} draggable={true} eventHandlers={{click:()=>{setEditPopup({type:'zone',id:z.id,data:z});setCtxMenu(null);}, dragend:(e)=>handleMarkerDragEnd('zone-center',z.id,e)}}>
                    ...
                    {!showRoute && bayWadeLines.map(wl=>{
                      ...
                      return <React.Fragment key={'wl'+wl.id}>
                        <Polyline positions={lineCoords} pathOptions={{color:wl.color,weight:3,opacity:0.9}} eventHandlers={{click:()=>{ if(editMode){setEditPopup({type:'wadeline',id:wl.id,data:wl});setCtxMenu(null);} }}}>
                    ...
                    {!showRoute && bayLaunches.map(l=>(
                      <Marker key={`l${l.id}`} position={bayConfig.toLatLng(l.position)} icon={launchIcon(l.type)} draggable={editMode} eventHandlers={{click:()=>{ if(editMode){setEditPopup({type:'launch',id:l.id,data:l});setCtxMenu(null);} }, dragend:(e)=>handleMarkerDragEnd('launch',l.id,e)}}>
                    ...
                    {!showRoute && bayPhotos.map(p=>(
                      <Marker key={`p${p.id}`} position={bayConfig.toLatLng(p.position)} icon={photoIcon()} draggable={editMode} eventHandlers={{click:()=>{ if(editMode){setEditPopup({type:'photo',id:p.id,data:p});setCtxMenu(null);} }, dragend:(e)=>handleMarkerDragEnd('photo',p.id,e)}}>
                    ...
                    {!showRoute && filtered.map(s=>(
                      <Marker key={`s${s.id}`} position={bayConfig.toLatLng(s.position)} icon={spotIcon(s.type,selSpot?.id===s.id)} draggable={editMode} eventHandlers={{click:()=>{ if(editMode){setEditPopup({type:'spot',id:s.id,data:s});setCtxMenu(null);} else openSpot(s); }, dragend:(e)=>handleMarkerDragEnd('spot',s.id,e)}}>
                  ...
                  {ctxMenu && editMode && <div style={{position:'absolute',left:ctxMenu.x+14,top:ctxMenu.y+52,zIndex:1000,background:C.card,border:'1px solid '+C.bdr2,borderRadius:12,padding:6,minWidth:180,boxShadow:'0 8px 32px #000a'}} onClick={e=>e.stopPropagation()}>
                    ...
                    {[{t:'wade',l:'Wade Spot',i:'\uD83D\uDEB6',c:C.amber},{t:'boat',l:'Boat Spot',i:'\uD83D\uDEA4',c:C.blue},{t:'kayak',l:'Kayak Spot',i:'\uD83D\uDEF6',c:C.green},{t:'wade-line',l:'Wade Line + Cast',i:'\uD83C\uDFA3',c:C.amber},{t:'wade-zone',l:'Wade Zone',i:'\uD83D\uDDFA',c:C.amber},{t:'launch-boat',l:'Boat Ramp',i:'\u2693',c:C.cyan},{t:'launch-kayak',l:'Kayak Launch',i:'\uD83D\uDEF6',c:C.teal},{t:'launch-drivein',l:'Drive-in Access',i:'\uD83D\uDE97',c:C.purple}].map(opt=><button key={opt.t} onClick={()=>handleAddFromCtx(opt.t)} style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'8px 10px',borderRadius:8,background:'transparent',border:'none',color:C.txt,cursor:'pointer',fontFamily:Fnt,fontSize:12,textAlign:'left'}} onMouseEnter={e=>e.currentTarget.style.background=C.card2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{width:28,height:28,borderRadius:6,background:opt.c+'25',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15}}>{opt.i}</span><div><div style={{fontWeight:600}}>{opt.l}</div></div></button>)}
                  ...
                  {/* EDIT POPUP FOR SELECTED MARKER */}
                  {editPopup && editMode && <div style={{position:'absolute',right:10,top:52,zIndex:1000,width:290,background:C.card,border:'1px solid '+C.bdr2,borderRadius:14,boxShadow:'0 8px 32px #000a',overflow:'hidden',maxHeight:420,overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
                    <div style={{padding:'10px 14px',borderBottom:'1px solid '+C.bdr,display:'flex',justifyContent:'space-between',alignItems:'center',background:C.cyan+'12',position:'sticky',top:0,zIndex:1}}>
                      <div style={{fontWeight:700,fontSize:13,display:'flex',alignItems:'center',gap:6}}><EditI s={14} c={C.cyan}/> Edit {editPopup.type==='spot'?'Spot':editPopup.type==='launch'?'Launch':editPopup.type==='zone'?'Zone':editPopup.type==='wadeline'?'Wade Line':'Photo'}</div>
                      <button onClick={()=>setEditPopup(null)} style={{background:'none',border:'none',color:C.dim,cursor:'pointer'}}><XI s={14}/></button>
                    </div>
                    <div style={{padding:14}}>
                      {editPopup.type==='spot' && <>
                        <Inp label="Spot Name" value={editPopup.data.name} onChange={e=>handleUpdateSpot(editPopup.id,{name:e.target.value})}/>
                        <Sel label="Type" value={editPopup.data.type} onChange={e=>handleUpdateSpot(editPopup.id,{type:e.target.value})} options={[{value:'wade',label:'Wade'},{value:'boat',label:'Boat'},{value:'kayak',label:'Kayak'}]}/>
                        <Inp label="Species (comma-sep)" value={editPopup.data.species?.join(', ')||''} onChange={e=>handleUpdateSpot(editPopup.id,{species:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)})}/>
                        <Sel label="Best Tide" value={editPopup.data.bestTide||'Any'} onChange={e=>handleUpdateSpot(editPopup.id,{bestTide:e.target.value})} options={[{value:'Incoming',label:'Incoming'},{value:'Outgoing',label:'Outgoing'},{value:'High',label:'High'},{value:'Low',label:'Low'},{value:'Moving',label:'Any Moving'},{value:'Any',label:'Any'}]}/>
                        <Inp label="Best Time" value={editPopup.data.bestTime||''} onChange={e=>handleUpdateSpot(editPopup.id,{bestTime:e.target.value})} placeholder="e.g. 5-9 AM"/>
                        <Inp label="Lures (comma-sep)" value={editPopup.data.lures?.join(', ')||''} onChange={e=>handleUpdateSpot(editPopup.id,{lures:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)})}/>
                        <div style={{marginBottom:12}}><Lbl>Notes</Lbl><textarea value={editPopup.data.desc||''} onChange={e=>handleUpdateSpot(editPopup.id,{desc:e.target.value})} style={{width:'100%',padding:8,borderRadius:8,background:C.card2,border:'1px solid '+C.bdr,color:C.txt,fontSize:12,fontFamily:Fnt,minHeight:50,resize:'vertical',outline:'none'}} placeholder="Notes about this spot..."/></div>
                      </>}
                      {editPopup.type==='launch' && <>
                        <Inp label="Name" value={editPopup.data.name} onChange={e=>handleUpdateLaunch(editPopup.id,{name:e.target.value})}/>
                        <Sel label="Type" value={editPopup.data.type} onChange={e=>handleUpdateLaunch(editPopup.id,{type:e.target.value})} options={[{value:'boat',label:'Boat Ramp'},{value:'kayak',label:'Kayak Launch'},{value:'drivein',label:'Drive-in Access'}]}/>
                        <Inp label="Notes" value={editPopup.data.notes||''} onChange={e=>handleUpdateLaunch(editPopup.id,{notes:e.target.value})} placeholder="Parking, fees, conditions..."/>
                      </>}
                      {editPopup.type==='zone' && <>
                        <Inp label="Zone Label" value={editPopup.data.label||''} onChange={e=>handleUpdateZone(editPopup.id,{label:e.target.value})}/>
                        <Sel label="Type" value={editPopup.data.type||'wade'} onChange={e=>handleUpdateZone(editPopup.id,{type:e.target.value,color:sc(e.target.value)})} options={[{value:'wade',label:'Wade'},{value:'kayak',label:'Kayak'},{value:'boat',label:'Boat'}]}/>
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                          <Inp label="Width" type="number" value={editPopup.data.rx||6} onChange={e=>handleUpdateZone(editPopup.id,{rx:+e.target.value})}/>
                          <Inp label="Height" type="number" value={editPopup.data.ry||4} onChange={e=>handleUpdateZone(editPopup.id,{ry:+e.target.value})}/>
                        </div>
                        <div style={{fontSize:11,color:C.dim,marginBottom:8}}>Drag the center marker on the map to reposition</div>
                      </>}
                      {editPopup.type==='wadeline' && <>
                        <Inp label="Line Label" value={editPopup.data.label||''} onChange={e=>handleUpdateWadeLine(editPopup.id,{label:e.target.value})}/>
                        <Inp label="Cast Range (yards)" type="number" value={editPopup.data.castRange||40} onChange={e=>handleUpdateWadeLine(editPopup.id,{castRange:+e.target.value})}/>
                        <div style={{fontSize:11,color:C.dim,marginBottom:8}}>{editPopup.data.points?.length||0} points - drag orange dots to reshape</div>
                        <Btn small style={{width:'100%',marginBottom:8}} onClick={()=>{
                          const pts = editPopup.data.points||[];
                          if(pts.length>=2){
                            const last=pts[pts.length-1];
                            const prev=pts[pts.length-2];
                            const np={x:last.x+(last.x-prev.x)*0.5,y:last.y+(last.y-prev.y)*0.5};
                            handleUpdateWadeLine(editPopup.id,{points:[...pts,np]});
                          }
                        }}><PlusI s={12}/> Add Point to End</Btn>
                      </>}
                      {editPopup.type==='photo' && <>
                        <Inp label="Caption" value={editPopup.data.caption||''} onChange={e=>setCommunityPhotos(prev=>prev.map(p=>p.id===editPopup.id?{...p,caption:e.target.value}:p))}/>
                        <div style={{fontSize:11,color:C.dim,marginBottom:8}}>Drag marker on map to reposition</div>
                      </>}
                      <div style={{padding:'8px 0',background:C.card2,borderRadius:8,textAlign:'center',marginBottom:10,fontFamily:FM,fontSize:11,color:C.cyan}}>
                        {editPopup.data.position ? (()=>{const[la,lo]=bayConfig.toLatLng(editPopup.data.position||{x:editPopup.data.cx||50,y:editPopup.data.cy||50});return la.toFixed(5)+', '+lo.toFixed(5);})() : editPopup.data.gps ? (typeof editPopup.data.gps==='string'?editPopup.data.gps:editPopup.data.gps.lat+', '+editPopup.data.gps.lng) : editPopup.data.cx ? (()=>{const[la,lo]=bayConfig.toLatLng({x:editPopup.data.cx,y:editPopup.data.cy});return la.toFixed(5)+', '+lo.toFixed(5);})() : ''}
                      </div>
                      <div style={{display:'flex',gap:6}}>
                        <Btn primary small style={{flex:1}} onClick={()=>{showT('Saved!');setEditPopup(null);}}><SaveI s={12} c={C.bg}/> Done</Btn>
                        <Btn small danger onClick={()=>handleDeleteMarker(editPopup.type==='wadeline'?'wadeline':editPopup.type,editPopup.id)}><TrashI s={12}/> Delete</Btn>
                      </div>
                    </div>
                  </div>}

                  {/* WADE LINE DRAWING CONTROLS */}
                  {drawingLine && editMode && <div style={{position:'absolute',bottom:10,left:'50%',transform:'translateX(-50%)',zIndex:1000,background:C.card,border:'1px solid '+C.green,borderRadius:12,padding:'8px 16px',boxShadow:'0 4px 20px #000a',display:'flex',alignItems:'center',gap:10}}>
